...
mkdir -p /home/hadoop/app/sh;mkdir -p $DFSDATA_PATH; mkdir -p $DFSDATA2_PATH; mkdir -p $ZOOKEEPERDLOGS_PATH; mkdir -p $ZOOKEEPERDATA_PATH;
chmod -R 755 $DFSDATA_PATH $DFSDATA2_PATH $ZOOKEEPERDLOGS_PATH $ZOOKEEPERDATA_PATH;

for i in $(cat $HADOOP_HOME/conf/hdfs); do
	echo "`date +%Y%m%d%t%T` envSet to $i "

	ssh -p 10022 $i "echo 'umask 0022'>>~/.bash_profile;"
	ssh -p 10022 $i "mkdir -p /home/hadoop/app/sh;mkdir -p $DFSDATA_PATH; mkdir -p $DFSDATA2_PATH; mkdir -p $ZOOKEEPERDLOGS_PATH; mkdir -p $ZOOKEEPERDATA_PATH; chmod -R 755 $DFSDATA_PATH $DFSDATA2_PATH $ZOOKEEPERDLOGS_PATH $ZOOKEEPERDATA_PATH;"
done
...
echo "`date +%Y%m%d%t%T` distHadoopNNCfgs cluster done ..."
}
distHadoopjtCfgs()
{
for j in 1 2 3; do
...
##cp:conf, cplatform.tools.jar, jdom.jar ojdbc14_g.jar,WorkManager.jar, hive-hwi-0.9.0.war hive-metastore-0.9.0.jar zookeeper-3.4.4.jar hbase-0.94.2-security.jar
...
#	scp -P 10022 $HIVE_HOME/lib/zookeeper-3.4.4.jar $HIVE_HOME/lib/hbase-0.94.2-security.jar $HIVE_HOME/lib/WorkManager.jar $HIVE_HOME/lib/ojdbc14_g.jar $HIVE_HOME/lib/jdom.jar $HIVE_HOME/lib/cplatform.tools.jar $HIVE_HOME/lib/hive-hwi-0.9.0.war $HIVE_HOME/lib/hive-metastore-0.9.0.jar hadoop@$i:$HIVE_HOME/lib/
done
...
for i in $(cat $ZOOKEEPER_HOME/conf/dist); do
	...
	ssh -p 10022 $i "rm -rf $ZOOKEEPER_HOME;tar zxf $ZOOKEEPER_HOME.tgz;echo '$j' > $ZOOKEEPER_HOME/conf/myid;cp -f $ZOOKEEPER_HOME/conf/myid $ZOOKEEPERDATA_PATH/;$ZOOKEEPER_HOME/bin/zkServer.sh start;mv -f $ZOOKEEPER_HOME.tgz ~/app"
	j=`expr $j + 1`
done
...
for i in $(cat $ZOOKEEPER_HOME/conf/dist); do
	...
	ssh -p 10022 $i "$ZOOKEEPER_HOME/bin/zkServer.sh restart"
#	sleep 3
...
echo "`date +%Y%m%d%t%T` distHbaseCfgs cluster done ..."
}
